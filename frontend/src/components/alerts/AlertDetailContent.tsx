import { CheckCircle2, ExternalLink, Siren } from "lucide-react";
...
/**
 * Alert investigation content shared by the routed detail page (mobile and
 * deep links) and the desktop Alert Queue split view.
 */
export function AlertDetailContent({ alertId }: AlertDetailContentProps) {
  const alertQuery = useAlertQuery(alertId);
    ...
    incidentMutation.mutate(
      ...
      },
      {
        onSuccess: (incident) => {
          ...
          setFeedback("Incident created from this alert.");
        },
        onError: (error) => {
          setFeedback(messageFrom(error));
        },
  ...
  return (
    <div className="alert-detail-content">
      <header className="alert-detail-content__header">
        <div>
      ...
      {feedback && (
        <p className="operation-feedback" role="status">
          ...
          {createdIncidentId && (
            <>
              {" "}
              <Link to={`/incidents/${createdIncidentId}`}>View incident</Link>
            </>
          )}
        </p>
      )}
